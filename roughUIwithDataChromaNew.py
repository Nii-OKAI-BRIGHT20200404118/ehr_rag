...
import chromadb
import llama_index
llama_index.analytics_enabled = False
...
class SyntheaParser:
    ...
    def parse_data(self) -> List[EHRDocument]:
        ...
        try:
            ...
            logger.error(f"Error parsing Synthea data: {e}")
            raise

    def _parse_conditions(self, ehr_documents):
        conditions_file = self.data_dir / 'conditions.csv'
        self._parse_file(conditions_file, ehr_documents, self._parse_condition)

    def _parse_encounters(self, ehr_documents):
        encounters_file = self.data_dir / 'encounters.csv'
        self._parse_file(encounters_file, ehr_documents, self._parse_encounter)

    def _parse_medications(self, ehr_documents):
        medications_file = self.data_dir / 'medications.csv'
        self._parse_file(medications_file, ehr_documents, self._parse_medication)

    def _parse_procedures(self, ehr_documents):
        procedures_file = self.data_dir / 'procedures.csv'
        self._parse_file(procedures_file, ehr_documents, self._parse_procedure)

    def _parse_file(self, file_path, ehr_documents, parse_function):
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if patient_id in ehr_documents:
                        parse_function(ehr_documents[patient_id], row)
        except Exception as e:
            logger.error(f"Error parsing file {file_path}: {e}")
            raise

    def _parse_condition(self, ehr_document, row):
        ehr_document.conditions.append({
            'start': row['START'],
            'stop': row['STOP'],
            'code': row['CODE'],
    ...
    def _parse_encounter(self, ehr_document, row):
        ehr_document.encounters.append({
            'id': row['Id'],
            'start': row['START'],
            'stop': row['STOP'],
    ...
    def _parse_medication(self, ehr_document, row):
        ehr_document.medications.append({
            'start': row['START'],
            'stop': row['STOP'],
            'code': row['CODE'],
            'description': row['DESCRIPTION'],
    ...
    def _parse_procedure(self, ehr_document, row):
        ehr_document.procedures.append({
            'date': row['DATE'],
            'code': row['CODE'],
            'description': row['DESCRIPTION'],
...
class EHRSummarizer:
    def __init__(self, config: OllamaConfig, persist_dir: str = "./chroma_db"):
        try:
            ...
            logger.error(f"Failed to initialize EHRSummarizer: {e}")
...
if __name__ == "__main__":
    try:
        ...
        ehr_data = load_synthea_data('E:/AI RESEEARCH NEW/ehr_rag/Data/syntheaNewData')  # Update this path
