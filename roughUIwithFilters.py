...
class EHRSummarizer:
    ...
    def summarize(self, task: str, patient_id: str, start_date: str = None, end_date: str = None, 
                  event_types: List[str] = None, min_cost: float = None, max_cost: float = None) -> str:
        try:
            filter_info = self._create_filter_info(start_date, end_date, event_types, min_cost, max_cost)
            ...
            prompt = (
                ...
                f"Apply the following filters to the data: {filter_info}\n"
    ...
    def _create_filter_info(self, start_date: str, end_date: str, event_types: List[str], min_cost: float, max_cost: float) -> str:
        filters = []
        if start_date:
        ...
        if min_cost is not None:
            filters.append(f"Minimum cost: ${min_cost:.2f}")
        if max_cost is not None:
            filters.append(f"Maximum cost: ${max_cost:.2f}")
        
        return "No filters applied." if not filters else "Filters: " + "; ".join(filters)
...
def generate_summary(patient_id: str, task: str, start_date: str = None, end_date: str = None, 
                     event_types: List[str] = None, min_cost: float = None, max_cost: float = None) -> str:
    if not patient_id:
    ...
    return summarizer.summarize(task, patient_id, start_date, end_date, event_types, min_cost, max_cost)
...
with gr.Blocks(title="EHR Summarizer", theme=gr.themes.Soft()) as app:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            start_date = gr.Textbox(label="Start Date (YYYY-MM-DD)", placeholder="e.g., 2020-01-01")
            end_date = gr.Textbox(label="End Date (YYYY-MM-DD)", placeholder="e.g., 2023-12-31")
        with gr.Column(scale=1):
            event_types = gr.CheckboxGroup(
                ["Conditions", "Encounters", "Medications", "Procedures"],
                label="Event Types"
            )
        with gr.Column(scale=1):
            min_cost = gr.Number(label="Minimum Cost ($)")
            max_cost = gr.Number(label="Maximum Cost ($)")
    ...
    def on_summarize_click(patient_id, task, custom_query, start_date, end_date, event_types, min_cost, max_cost):
        query_map = {
            ...
            "Summarize recent encounters and their reasons": "summarize the patient's medical encounters, including dates, types, reasons for visit, and outcomes.",
            "List current medications with dosages and costs": "list all medications prescribed to the patient, including dosages, start dates, and total costs.",
        ...
        return generate_summary(patient_id, query, start_date, end_date, event_types, min_cost, max_cost)
    
    summarize_btn.click(
        on_summarize_click,
        inputs=[patient_id, task, custom_query, start_date, end_date, event_types, min_cost, max_cost],
        outputs=output
    )

    gr.Markdown("## Note")
    gr.Markdown("This application uses synthetic data generated by Synthea. Patient IDs are in the format P0001, P0002, etc.")
    gr.Markdown("Please enter dates in the format YYYY-MM-DD (e.g., 2023-01-15)")
